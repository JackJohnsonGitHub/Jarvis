...
export function registerWorkflowTriggerCommand(pi, state, settingsStore = DEFAULT_SETTINGS_STORE) {
    pi.registerCommand?.("workflows-trigger", {
        ...
        async handler(args, _ctx) {
            ...
            await say(`Workflows keyword trigger is ${state.keywordTriggerEnabled ? "on" : "off"}. Changes are saved for new sessions. Usage: /workflows-trigger on | off | status`);
        },
    });
}
...
function loadInitialKeywordTrigger(settingsStore) {
    try {
...
function persistKeywordTrigger(settingsStore, enabled) {
    try {
