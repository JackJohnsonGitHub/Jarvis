...
const ULTRA_DIRECTIVE = "Effort: ULTRA. Be exhaustive — fan out widely (more reviewers/judges, deeper loopUntilDry rounds, a completenessCheck at the end), set a generous tokenBudget and a high maxAgents on the workflow tool call, and prefer the big tier for synthesis.";
...
export function registerEffortCommand(pi, state) {
    ...
    pi.registerCommand("ultracode", {
        description: "Ultracode: standing maximal-effort mode — auto-arms an exhaustive workflow for substantive messages. /ultracode off to stop.",
        async handler(args, _ctx) {
            const arg = args.trim().toLowerCase();
            const say = (content) => pi.sendMessage({ customType: "effort", content, display: true });
            if (arg === "off") {
