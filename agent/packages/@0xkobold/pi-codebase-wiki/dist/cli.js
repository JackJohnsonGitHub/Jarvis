...
import { loadConfig, wikiExists, getWikiPath, ensureWikiDirs, initWiki, ingestCommits, ingestFileTree, updateIndex, searchWiki, lintWiki, formatLintResult, enrichAllEntities, getRecentCommits, getAllCommits, getCurrentBranch, getLatestHash, } from "./core/index.js";
import { toSlug, validateSlug, formatWikiDate, getDirectoryForPageType, DOMAIN_PRESETS, DEFAULT_PAGE_TYPES } from "./shared.js";
import { mergePages, updatePages, splitPage } from "./operations/resolve.js";
import { findContradictionsDetailed } from "./core/staleness.js";
// ============================================================================
// SHARED HELPERS
// ============================================================================
const DEFAULT_CONFIG = loadConfig();
async function getStore(rootDir) {
...
function posArg(ctx, index, fallback) {
    const rest = (ctx.args?.rest ?? []);
...
kapy()
    ...
    const wikiDir = ctx.args.dir || ".codebase-wiki";
    const domain = ctx.args.domain || "codebase";
    const mode = ctx.args.mode || "auto";
    ...
    const config = { ...DEFAULT_CONFIG, wikiDir, domain, pageTypes, ingestionMode: mode };
    ...
    const since = ctx.args.since || "1 week ago";
    ...
    const type = ctx.args.type || "module";
    ...
    const context = ctx.args.context;
    const decision = String(ctx.args.choice ?? ctx.args.d ?? "");
    const status = ctx.args.status || "Proposed";
    const alternatives = ctx.args.alternatives;
    const store = await getStore(rootDir);
    if (!store)
        return;
    try {
    ...
    const rootDir = process.cwd();
    const since = ctx.args.since || "1 week ago";
    const format = ctx.args.format || "keepachangelog";
    ...
    if (wikiExists(rootDir, DEFAULT_CONFIG.wikiDir)) {
        ...
        if (existingContent) {
            const existingHashes = new Set([...existingContent.matchAll(/Commit: `([a-f0-9]{7,40})`/g)].map(m => m[1].slice(0, 7)));
            const newLines = lines.filter(line => {
                const hashMatch = line.match(/Commit: `([a-f0-9]{7,40})`/);
                if (hashMatch)
                    return !existingHashes.has(hashMatch[1].slice(0, 7));
                return true;
            });
    ...
    const port = ctx.args.port || 3000;
    const shouldOpen = ctx.args.open;
    ...
    const contentFlag = ctx.args.content;
    const filePath = ctx.args.file;
    const url = ctx.args.url;
    const pageType = ctx.args["page-type"];
    if (!wikiExists(rootDir, DEFAULT_CONFIG.wikiDir)) {
        console.error("❌ Wiki not initialized. Run `wiki init` first.");
        process.exit(1);
    }
    ...
    const result = ingestSource(wikiPath, rootDir, sourceType, title, content, store, {
        url,
        pageType,
    });
    ...
    const rootDir = process.cwd();
    const lastN = ctx.args.last || 10;
    const typeFilter = ctx.args.type;
    const since = ctx.args.since;
    if (!wikiExists(rootDir, DEFAULT_CONFIG.wikiDir)) {
        console.error("\u274c Wiki not initialized. Run `wiki init` first.");
        process.exit(1);
    }
    ...
    const rootDir = process.cwd();
    const strategy = ctx.args.strategy;
    const pageArgs = ctx.args._.filter(a => !a.startsWith("-"));
    if (!wikiExists(rootDir, DEFAULT_CONFIG.wikiDir)) {
    ...
    try {
        ...
        if (strategy === "merge") {
            ...
            const targetId = ctx.args.target || pageArgs[1];
            ...
            const sourceId = pageArgs[0];
            const intoStr = ctx.args.into;
            const titlesStr = ctx.args.titles;
            if (!sourceId || !intoStr) {
