...
export function createLensDiagnosticsTool(cacheManager, getCwd) {
    return {
        name: "lens_diagnostics",
        label: "Project Diagnostics",
        description: "Query pi-lens's current diagnostic state without re-running LSP or dispatch. " +
...
function formatDeltaMode(cacheManager, cwd, severity) {
    ...
    if (lines.length === 0) {
        ...
        return { content: [{ type: "text", text }], details: { mode: "delta", warnings: 0 } };
    }
...
function formatAllMode(cwd, severity) {
    ...
    if (withIssues.length === 0) {
        ...
        return { content: [{ type: "text", text }], details: { mode: "all", filesChecked: summaries.length } };
    }
    // Sort: blocking first, then errors, then warnings
    const sorted = withIssues.sort((a, b) => b.blocking - a.blocking ||
        b.errors - a.errors ||
