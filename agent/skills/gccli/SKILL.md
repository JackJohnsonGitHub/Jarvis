...
`gccli` has no native RSVP command, so accepting invites is done by copying the event to the vikram@malkans.net calendar account.

When the user asks to **accept calendar invite(s)**, do the following by default:

1. Find the invite email(s) with `gmcli` (e.g. `gmcli <email> search "subject:Invitation"` or `filename:ics`).
2. Read the thread (`gmcli <email> thread <threadId>`) to extract:
   - Event summary/title
   - Start date/time
   - End date/time
   - Description and location (if relevant)
3. Create a copy of the event on vikram@malkans.net's calendar:
   ```bash
   gccli vikram@malkans.net create primary \
     ...
     [--location "<location>"]
   ```
4. Confirm the event was created successfully.

Notes:
- This creates a copy of the event on the vikram@malkans.net calendar; it does NOT send an "Accepted" RSVP to the organizer.
- For recurring events, copy only the next occurrence or the series pattern as needed.
- The vikram@malkans.net account must be configured with `gccli accounts list`.
