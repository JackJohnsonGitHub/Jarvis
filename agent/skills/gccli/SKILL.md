...
`gccli` has no native RSVP command, so accepting invites is done by copying the event to the <your-account> calendar account.
...
3. Create a copy of the event on <your-account>'s calendar:
   ```bash
...
Notes:
- This creates a copy of the event on the <your-account> calendar; it does NOT send an "Accepted" RSVP to the organizer.
- For recurring events, copy only the next occurrence or the series pattern as needed.
- The <your-account> account must be configured with `gccli accounts list`.
